...
describe('RegisterUseCase', () => {
	...
	describe('execute', () => {
		it('throw PreconditionFailedException when UserService.getMany return user with same username', async () => {
			...
			await expect(user).rejects.toThrow(PreconditionFailedException);
			await expect(user).rejects.toMatchObject({ response: { username: 'already exists' } });
		});
		...
		it('throw PreconditionFailedException when UserService.getMany return user with same email', async () => {
			...
			await expect(user).rejects.toThrow(PreconditionFailedException);
			await expect(user).rejects.toMatchObject({ response: { email: 'already exists' } });
		});
