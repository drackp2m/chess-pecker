...
describe('RefreshSessionUseCase', () => {
	...
	// `execute` is synchronous, so the exception surfaces as a throw, not a rejection.
	const captureExecuteError = (): unknown => {
		try {
			useCase.execute();
		} catch (error: unknown) {
			return error;
	...
	describe('execute', () => {
		it('throw UnauthorizedException when the request missing cookies', () => {
			const error = captureExecuteError();

			expect(error).toBeInstanceOf(UnauthorizedException);
			expect(error).toMatchObject({ response: { refreshToken: 'jwt must be provided' } });
		});

		it('throw UnauthorizedException when token is invalid', () => {
			request.signedCookies['x-jwt-refresh-token'] = 'wrong-refresh-token';

			const error = captureExecuteError();

			expect(error).toBeInstanceOf(UnauthorizedException);
			expect(error).toMatchObject({ response: { refreshToken: 'jwt malformed' } });
