...
export function buildActivityGrid(
	...
	const { min, max } = countBounds(counts);

	return Array.from({ length: totalWeeks }, (_unused, week) =>
		Array.from({ length: DAYS_PER_WEEK }, (_unused, weekday) =>
			toCell(addUtcDays(start, week * DAYS_PER_WEEK + weekday), rangeStart, end, counts, min, max),
		),
	);
}

/** Exact day count of the last `months` calendar months, today included. */
export function activityRangeDays(months: number): number {
	const end = utcMidnight(new Date());
...
export function weekdayAbbreviations(locale: string): readonly (string | null)[] {
	const monday = mostRecentMonday(utcMidnight(new Date()));
...
function toCell(
	...
	return { date: isoDate, count, level: toLevel(count, min, max) };
}

/** Scales onto the 4 filled levels between the lowest and highest count actually seen. */
function toLevel(count: number, min: number, max: number): ActivityCell['level'] {
	if (0 === count) {
		return 0;
	}
	...
	return (1 + Math.round(ratio * 3)) as ActivityCell['level'];
}

function countBounds(counts: ReadonlyMap<string, number>): { min: number; max: number } {
	const values = [...counts.values()].filter((count) => 0 < count);
