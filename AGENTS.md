...
CI is split in two. `.github/workflows/ci.yml` runs on **pull requests to `main`** with three parallel jobs: `lint` (`pnpm run lint` over the whole repo), `web` (Vitest + production build) and `api` (`tsc --noEmit -p apps/api/tsconfig.json`, Vitest unit project, `test:integration`, `nest build`). `.github/workflows/deploy.yml` runs on **push to `main`** and does no checking at all — semantic-release, then build, then deploy to Pages; `HUSKY=0` is set during the release commit to skip local hooks.
